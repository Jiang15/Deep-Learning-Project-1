...
class Siamese(nn.Module):
    def __init__(self,  weight_sharing_CNN, weight_sharing_FC , auxiliary_loss):

        super().__init__()
        self.weight_sharing_CNN = weight_sharing_CNN
        self.weight_sharing_FC = weight_sharing_FC
        self.auxiliary_loss = auxiliary_loss
        # self.CNNnet1 = nn.Sequential(nn.Conv2d(1, 32, 3),  nn.ReLU(), nn.BatchNorm2d(32), nn.MaxPool2d(2,2), nn.Conv2d(32, 64, 3, padding=1),  nn.ReLU(), nn.BatchNorm2d(64), nn.MaxPool2d(2,1))
        # self.CNNnet2 = nn.Sequential(nn.Conv2d(1, 32, 3), nn.ReLU(), nn.BatchNorm2d(32), nn.MaxPool2d(2,2),  nn.Conv2d(32, 64, 3, padding=1),  nn.ReLU(), nn.BatchNorm2d(64), nn.MaxPool2d(2,1))
        # self.FCnet1 = nn.Sequential(nn.Linear(64 * 5 * 5, 256), nn.ReLU(), nn.Linear(256, 10),nn.Softmax(-1))
        # self.FCnet2 = nn.Sequential(nn.Linear(64 * 5 * 5, 256), nn.ReLU(), nn.Linear(256, 10),nn.Softmax(-1))
        # #self.combine = nn.Sequential(nn.ReLU(), nn.Linear(64*5*5*2, 256), nn.ReLU(), nn.Linear(256, 2))
        # self.combine = nn.Sequential(nn.Linear(10*2,2), nn.Softmax(-1))
